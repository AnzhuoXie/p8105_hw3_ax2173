...
<img src="p8105_hw3_ax2173_files/figure-gfm/unnamed-chunk-7-1.png" width="90%" />
...
<img src="p8105_hw3_ax2173_files/figure-gfm/unnamed-chunk-8-1.png" width="90%" />

ny_noaa %\>% filter(month == ‘01’ \| month == ‘07’) %\>%
group_by(station, month) %\>% summarize(average_tmax = mean(tmax, na.rm
= TRUE)) %\>% ggplot(aes(x = station, y = average_tmax, color =
station)) + geom_point() + facet_grid(. \~ month) + labs( title = ‘The
scatter plot of average max temperature in Januart and February’, x =
‘Station’, y = ‘Average Max Temperature’, caption = ‘Data is from noaa
package’)
